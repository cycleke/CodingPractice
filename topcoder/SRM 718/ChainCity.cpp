...
public:
    ...
    int findMin(vector<int> dist, int k) {
        ...
        for (int x : dist) {
            len += x;
            distance.push_back(len);
        }
        for (long long &x : distance) x <<= 1;
        long long lower = 0, upper = len;
        while (lower <= upper) {
            long long middle = (lower + upper) >> 1;
            if (check(middle)) {
                upper = middle - 1;
            } else {
                lower = middle + 1;
