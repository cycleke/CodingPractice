...
public:
	long long validNumbers(int digits, vector<string> specialPrefixes) {
		...
		long long res = p10[digits];
		for (int i = 0; i < n; ++i) {
			bool visable = true;
			for (int j = 0; j < n; ++j)
				if (specialPrefixes[i].size() < specialPrefixes[j].size() && specialPrefixes[j].substr(0, specialPrefixes[i].size()) == specialPrefixes[i]) {
					visable = false;
					break;
				}
			if (visable) res -= p10[digits - specialPrefixes[i].size()];
		}
